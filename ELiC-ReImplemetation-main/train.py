...
class PSNRAverageMeter(AverageMeter):
    """Compute running average for PSNR."""

    def __init__(self):
        super().__init__()

    def update(self, val, n=1):
        # PSNR is usually in decibels, so we don't need to average in the same way as other metrics
        self.sum += val * n
        self.count += n
        self.avg = self.sum / self.count
...
def train_one_epoch(
    ...
    train_psnr = PSNRAverageMeter()
    # total_bits = 0  # 用于计算总比特数
    start = time.time()
    ...
    for i, d in enumerate(train_dataloader):
        ...
        # Compute PSNR
        with torch.no_grad():
            psnr_val = psnr(out_net['x_hat'], d)
        
        train_psnr.update(psnr_val)
        ...
        # total_bits += num_pixels * (out_criterion["bpp_loss"].item())
    ...
    # avg_bit_rate = total_bits / len(train_dataloader.dataset)
    # print(f"Average Bit-Rate for Epoch {epoch}: {avg_bit_rate:.2f} bits per pixel")
    ...
    logging.info(
        ...
        f"\tPSNR: {train_psnr.avg:.2f} |"
        # f"\tBpp: {avg_bit_rate:.2f} |"
    ...
    return train_loss.avg, train_bpp_loss.avg, train_mse_loss.avg, train_psnr.avg
...
def test_epoch(epoch, test_dataloader, model, criterion):
    ...
    with torch.no_grad():
        for d in test_dataloader:
            ...
            test_psnr.update(psnr(out_net['x_hat'], d))

    # Calculate average bit-rate
    # avg_bit_rate = total_bits / len(test_dataloader.dataset)
    ...
    return loss.avg, bpp_loss.avg, mse_loss.avg, test_psnr.avg
...
def main(argv):
    ...
    for epoch in range(last_epoch, args.epochs):
        ...
        train_loss, train_bpp, train_mse, train_psnr = train_one_epoch(
            net,
            criterion,
            train_dataloader,
            optimizer,
